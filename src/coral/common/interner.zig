...
pub fn internAndGetBitSet(
    ...
    for (strings) |s| {
        // intern() is idempotent — returns existing index or adds a new entry.
...
// ============================================================
// BitSetConstraint — runtime-parameterised vtable for DynamicBitSetUnmanaged
// ============================================================
//
// Usage:
//   const vtable = interner.bitSetConstraint();
//   // Then use &vtable as the constraint pointer in an Accessor.
//
// String path (setFn / getFn):
//   Input:  comma-separated capability names, e.g. "compile,link,test"
//   Output: same comma-separated format
//
// Binary path (setBinaryFn / getBinaryFn):
//   Wire format: [word_count: u32 LE][word0: u64 LE, word1: u64 LE, ...]
//   where words are the raw usize words of the DynamicBitSetUnmanaged.
//
// convertFn:
//   Translates a bitset from a foreign interner into this interner's bit
//   assignments by matching string capability names.  Used for DAG duck-typing
//   across module boundaries.
//
// releaseFn:
//   Calls DynamicBitSetUnmanaged.deinit on the field.
...
// ── String-path helpers for BitSetConstraint ─────────────────────────────────
// These are public so that TargetSchema (in target.zig) can call them directly
// when it has both the vtable context and the allocator at hand.

/// Parse a comma-separated capability string into a bitset.
...
/// Serialise a bitset to a comma-separated capability string.
...
test "StringInterner bitset creation" {
    var interner = StringInterner.init(testing.allocator);
    defer interner.deinit();
