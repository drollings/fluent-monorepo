...
const strutil = @import("string.zig");

pub const CommonArgs = args.CommonArgs;
pub const parseCommonArgs = args.parseCommonArgs;

pub const containsIgnoreCase = strutil.containsIgnoreCase;
pub const containsWord = strutil.containsWord;
pub const containsAny = strutil.containsAny;
pub const containsAnyWord = strutil.containsAnyWord;
pub const hasExtension = strutil.hasExtension;
pub const isPathToken = strutil.isPathToken;
...
fn llmContainsIgnoreCase(haystack: []const u8, needle: []const u8) bool {
    return strutil.containsIgnoreCase(haystack, needle);
}
...
test "LlmClient init with v1/completions URL converts to v1/chat/completions" {
    const allocator = std.testing.allocator;
    const config = LlmConfig{
