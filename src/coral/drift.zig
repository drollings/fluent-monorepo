...
//!
//! Implements the Coral Context DRIFT algorithm using bitwise set operations
//! instead of LLM-based decomposition.  Given a set of *needed* capabilities
//! and the set of *available* capabilities resolved so far, DRIFT computes
//! the missing set as `needed & ~available` and maps each missing bit back to
//! its interned string name to produce exact follow-up queries.
//!
...
/// Manages drifted bit sets with fixed-size buffers; owned by the process; ensures consistent state across operations.
...
test "BitSetDrift.generateFollowUps: produces follow-ups for missing bits" {
    ...
    // Each follow-up is "Provide <name>"
    for (followups) |fq| {
