...
test "CallExtractor: empty file produces no calls" {
    ...
    var extractor = try CallExtractor.init(allocator, "test.zig", source);
    defer extractor.deinit();
...
test "CallExtractor: buildImportMap finds @import aliases" {
    ...
    const source: [:0]const u8 =
        ...
        \\pub fn foo() void {}
    ;
    var extractor = try CallExtractor.init(allocator, "foo.zig", source);
    defer extractor.deinit();
...
test "CallExtractor: direct function calls extracted with high confidence" {
    ...
    const source: [:0]const u8 =
        ...
        \\fn inner() void {}
    ;
    var extractor = try CallExtractor.init(allocator, "t.zig", source);
    defer extractor.deinit();
