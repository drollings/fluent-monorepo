...
/// Tracks files that need comment-related processing during a gen run.
/// Used to coordinate the multi-phase comment sync pipeline.
...
/// Write generated comments from JSON back to source files.
/// Returns an owned slice of filepaths that were modified (caller must free).
...
/// Re-parse source and update line numbers in JSON.
/// Called after comments are written to source and after fmt runs.
