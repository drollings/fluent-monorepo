...
const comment_inserter = @import("inserter.zig");
const core = @import("core.zig");
const header_generator = @import("header.zig");
    ...
    pub fn processFile(self: *CommentSyncProcessor, filepath: []const u8) !CommentSyncResult {
        ...
        for (sorted) |member| {
            ...
            if (existing_comment == null) {
                ...
                var check = core.checkCommentStaleness(existing_comment.?, member, "");
                ...
                if (!check.needs_regeneration) {
                    ...
                    if (core.isHashStale(stored_hash, member.match_hash)) {
