...
mod tests {
    ...
    fn test_describable() {
        ...
        assert_eq!(desc["value"], 42);
    }

    #[test]
    fn test_work_unit_execute() {
        let comp = TestComponent {
            name: ArcIntern::from("test"),
            value: 42,
        };
        ...
        assert_eq!(output.message, "computed: 84");
    }
    ...
    fn test_component_trait_object() {
        ...
        let desc = boxed.describe();
        assert_eq!(desc["value"], 10);
    }
    ...
    fn test_work_output_helpers() {
        let ok = WorkOutput::ok("done");
        assert!(ok.success);
        assert_eq!(ok.message, "done");

        let fail = WorkOutput::fail("error");
        assert!(!fail.success);
    }
}
