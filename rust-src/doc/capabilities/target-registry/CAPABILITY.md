...
- `common/src/registry.rs` — `Target` (bon::Builder), `TargetRegistry` (basic)
- `common/src/interner.rs` — `CapabilityRegistry` (RwLock + double-checked locking)
- `dag/src/target.rs` — re-exports `Target` from `common`
- `dag/src/registry.rs` — `TargetRegistry` with `find_providers`, `get_providers`, `essential_targets`
